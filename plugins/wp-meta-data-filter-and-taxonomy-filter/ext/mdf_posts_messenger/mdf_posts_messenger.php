...
class MDF_POSTS_MESSENGER {
    ...
    public function __construct() {
        ...
        $this->subscribe_lang = __('Subscription', 'meta-data-filter');
    ...
    public function mdf_unsubscr() {
        ...
        if ($subscr[$sanit_get['key']]['secret_key'] == $sanit_get['mdf_skey']) {
            ...
            // $data['text']=__('You unsubscribed from the newsletter.','meta-data-filter'); 
        } else {
            //$data['text']=__('This link does not work.','meta-data-filter'); // if you want use another message
        }
        $data['text'] = __('You are unsubscribed from the future products newsletters.', 'meta-data-filter');
    ...
    public function mdf_init_js_css() {
        ...
        $translation_array = array(
            'mdf_confirm_lang' => __('Are you sure?', 'meta-data-filter')
        );
    ...
    public function create_new_email($data) {
        ...
        $headers[] = __('From: ','meta-data-filter') . get_bloginfo('name') . ' <no-reply@' . $site_name . '>';
        //+++
        // send email
            ...
            <div data-css-class="mdf_posts_messenger_container" class="mdf_posts_messenger_container mdf_container ">
                <div class="mdf_container_inner">
                    ...
                    <div class="mdf_add_subscr_cont" style="display: <?php echo $visible ?>" >
                        <input name="mdf_add_subscr_messenger" data-count="<?php echo $subscr_count ?>" type="button" id="mdf_add_subscr" data-user="<?php echo $cur_user_id ?>" value="<?php _e('Subscribe on the current search request', 'meta-data-filter') ?>"  >
                    </div>
