           ...
           // echo"<br>*********<br>";
            ...
            switch ($val['type']){
                ...
                case "calendar":
                    ...
                    $to="";
                     if(!empty(${$key}['from'])){
                        $from= __("From:",'meta-data-filter'). date("d.m.y",(int)${$key}['from']);
                       
                     }
                     if(!empty(${$key}['to'])){
                        $to= __("to:",'meta-data-filter'). date("d.m.y",(int)${$key}['to']);
                     }
                    ...
                    $to="";
                     if(!empty(${$key}['from']) OR ${$key}['from']==0 ){
                        $from= __("From:",'meta-data-filter').$val["range_select_prefix"].${$key}['from'].$val["range_select_postfix"];
                     }
                     if(!empty(${$key}['to'])){
                        $to= __("to:",'meta-data-filter').$val["range_select_prefix"].${$key}['to'].$val["range_select_postfix"];
                     }
                    ...
                    case "by_author":
                        ...
                        if(!empty($user_name)){
                            ?>             
                             <span><?php printf(__("Author: %s",'meta-data-filter'),$user_name)  ?></span>
                            <?php
                        }
                    ...
                    if(count($range)>1 AND( !empty($range[1])AND $range[1]!=0 )  ){
                            $from=sprintf(__("From:%s %s %s",'meta-data-filter'),$val["slider_prefix"],$range[0],$val["slider_postfix"]); 
                            $to=sprintf(__("to:%s %s %s",'meta-data-filter'),$val["slider_prefix"], $range[1],$val["slider_postfix"]);
