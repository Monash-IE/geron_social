...
<div class="mdf_products" style="width: 80%; margin: 0 auto; background: #fff; ">
    ...
    <?php if ($last_email) { ?>
        <div class="last_email"><?php _e('Attention! This is the last email. If you want to continue get such emails -> Go by next link and subscribe again', 'meta-data-filter') ?>
            - <a href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php _e('Subscribe', 'meta-data-filter'); ?></a>
        </div>
    <?php } ?>
	...
	if ($i < count($posts->posts)) {
		...
		<p style="text-align: center;font-size: 18px;" ><a style="padding: 4px;border: 2px solid #557DA1; text-decoration: none;" href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php _e('See more...', 'meta-data-filter') ?></a></p>
	    </div> 
	<?php
	}
    } else {
	echo __('No posts found','meta-data-filter');
    }
    wp_reset_postdata();
   
    ?>
</ul><!--/.products-->
<div class="woof_subscr"><p><?php _e('If you want to Unsubscribe from this newsletter', 'meta-data-filter') ?> - <a href="<?php echo $unsobscr_link ?>"><?php _e('unsubscribe', 'meta-data-filter') ?></a> </p>
