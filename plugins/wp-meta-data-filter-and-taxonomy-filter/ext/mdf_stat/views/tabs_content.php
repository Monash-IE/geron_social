...
<table class="form-table">
    <tbody>
        ...
        <tr valign="top">
            <th scope="row">
                <a href="javascript:void(0);" id="mdf_show_stat_options"  class="button"><?php _e('Show/Hide statistic options', 'meta-data-filter') ?></a>
            </th>
            <td>
                <p class="description" style="color:red;">
                    ...
                    $stat_activated_mode = array(
                        0 => __('Disabled', 'meta-data-filter'),
                        1 => __('Enabled', 'meta-data-filter')
                    );

                    if (!$is_enabled) {
                        _e('Statistics collection is not enabled. Click on the previous button to see activation drop-down and DataBase options.', 'meta-data-filter');
                    }
                    ?>
                </p>
            </td>
        ...
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("Statistic", 'meta-data-filter') ?>:</label></th>
            <td>               
                ...
                <p class="description"><?php _e('Should be enabled to start assembling.', 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("Server options for statistic", 'meta-data-filter') ?>:</label></th>
            <td>
                <?php
                ...
                if ($is_enabled AND (empty($server_options['host']) OR empty($server_options['host_user']) OR empty($server_options['host_db_name']) OR empty($server_options['host_pass']))) {
                    echo '<div class="error"><p class="description">' . sprintf(__('Statistic -> Statistic options -> "Stat server options" inputs should be filled in by right data, another way not possible to collect statistical data!', 'meta-data-filter')) . '</p></div>';
                }
                ?>


                <label style="margin-bottom: 5px; display: inline-block;"><?php _e('Host', 'meta-data-filter') ?></label>:
                ...
                <br />
                <label style="margin-bottom: 5px; display: inline-block;"><?php _e('User', 'meta-data-filter') ?></label>:
                ...
                <br />
                <label style="margin-bottom: 5px; display: inline-block;"><?php _e('DB Name', 'meta-data-filter') ?></label>:
                ...
                <br />
                <label style="margin-bottom: 5px; display: inline-block;"><?php _e('Password', 'meta-data-filter') ?></label>:
                ...
                <br/>
                <span id="mdf_stat_connection"  class="button"><?php _e('Check DB connection', 'meta-data-filter') ?></span>
                <p class="description"><?php _e('This data is very important for assembling statistics data, so please fill fields very responsibly. To collect statistical data uses a separate MySQL table.', 'meta-data-filter') ?>
        ...
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("Collect Statistic for", 'meta-data-filter') ?>:</label></th>
            <td>
                <?php
                ...
                if (is_array($data['post_types']) AND ! empty($data['post_types'])) {
                    ...
                    <p class="description"><?php _e('Select taxonomies and metadata which you want to track for each post type.', 'meta-data-filter') ?></p>
                ...
                foreach ($data['post_type_for_stat'] as $post_type) {
                    ...
                    <div class="mdf_stat_post_type">
                        ...
                        if (!empty($all_items)) {
                            ?>
                            <label><?php _e("Taxonomies:", 'meta-data-filter'); ?></label><br>
                        ...
                        <?php } ?>
                        <label><?php _e("Meta data:", 'meta-data-filter'); ?></label><br>
                        ...
                        <p class="description"><?php _e("Insert meta keys separated by end of line (press enter button).", 'meta-data-filter') ?></p>
        ...
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e('Max requests per unique user', 'meta-data-filter') ?></label></th>
            <?php
            ...
            <td>
                ...
                <p class="description"><?php _e("How many search requests will be catched and written down into the statistical mySQL table per 1 unique user before cron will assemble the data ", 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("Max deep of the search request", 'meta-data-filter') ?></label></th>
            <?php
            ...
            <td>
                ...
                <p class="description"><?php _e("How many taxonomies per one search request will be written down into the statistical mySQL table for 1 unique user. The excess data will be truncated! Number 5 is recommended. More depth - more space in the DataBase will be occupied by the data. ", 'meta-data-filter') ?></p>
            </td>
        </tr>

        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("How to assemble statistic", 'meta-data-filter') ?></label></th>
            <td>
                <?php
                $cron_systems = array(
                    0 => __('WordPress Cron', 'meta-data-filter'),
                    1 => __('External Cron', 'meta-data-filter')
                );
        ...
        <tr valign="top" class="mdf_stat_option cron_sys_1" style="visibility: <?php echo($cron_system == 1 ? 'visible' : 'hidden') ?>;">
            ...
            <td>
                ...
                <p class="description"><?php printf(__("Enter any random text in the field and use it in the external cron with link like: %s", 'meta-data-filter'), get_site_url() . '?mdf_stat_collection=' . $data['cron_secret_key']) ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option cron_sys_0"style="visibility: <?php echo($cron_system == 0 ? 'visible' : 'hidden') ?>;">
            <th scope="row">
                <label><?php _e("WordPress Cron period", 'meta-data-filter') ?></label></th>
            <td>
                <?php
                $wp_cron_periods = array(
                    'hourly' => __('hourly', 'meta-data-filter'),
                    'twicedaily' => __('twicedaily', 'meta-data-filter'),
                    'daily' => __('daily', 'meta-data-filter'),
                    'week' => __('weekly', 'meta-data-filter'),
                    'month' => __('monthly', 'meta-data-filter'),
                        //'min1' => __('min1', 'meta-data-filter')
                );
                ...
                <p class="description"><?php _e('12 hours recommended', 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" class="mdf_stat_option">
            <th scope="row"><label><?php _e("Max terms or taxonomies per graph", 'meta-data-filter') ?></label></th>
            <?php
            ...
            <td>
                ...
                <p class="description"><?php _e("How many taxonomies and terms to show on the graphs. Use no more than 10 to understand situation with statistical data", 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <?php global $wp_locale; ?>
        <tr valign="top" >
            <th scope="row"><label><?php _e("Select period:", 'meta-data-filter') ?></label></th>
            <td>
                <?php if (!empty($stat_min_date)): ?>
                    <div style="font-size: 12px; font-style: italic;"><?php printf(__('(Statistic collected from: %s %d)', 'meta-data-filter'), $wp_locale->get_month($stat_min_date[1]), $stat_min_date[0]) ?></div>
                <?php endif; ?>

                <input type="hidden" id="mdf_stat_calendar_from" value="0" />
                <input type="text" readonly="readonly" style="font-size: 1.2em;" class="regular-text mdf_stat_calendar mdf_stat_calendar_from" placeholder="<?php _e('From', 'meta-data-filter') ?>" />
                &nbsp;
                <input type="hidden" id="mdf_stat_calendar_to" value="0" />
                <input type="text" readonly="readonly" style="font-size: 1.2em;" class="regular-text mdf_stat_calendar mdf_stat_calendar_to" placeholder="<?php _e('To', 'meta-data-filter') ?>" /><br />

                <br />
                <p class="description"><?php _e("Select the time period for which you want to see statistical data", 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" >
            <th scope="row"><label><?php _e("Statistical parameters:", 'meta-data-filter') ?></label></th>
            <td>
                <?php
                ...
                <?php } else {
                    ?> <p class="description"><?php _e("Choose post type in the statistics settings!", 'meta-data-filter') ?></p> <?php }
                ?>
                <a href="javascript: mdf_stat_calculate();" class="button button-primary button-large"><?php _e('Calculate Statistics', 'meta-data-filter') ?></a><br />
                <p class="description"><?php _e('Select taxonomy(-ies) and/or meta_data (combination) OR leave this field empty to see general data for all the most requested taxonomies', 'meta-data-filter') ?></p>
            </td>
        </tr>
        <!--+++++++++++++-->
        <tr valign="top" >
            <th scope="row"><label><?php _e("Graphics:", 'meta-data-filter') ?></label></th>
            <td>
                <div class="mdf-control mdf-upload-style-wrap" style="width: 100%;">
                    ...
                    <div id="mdf_stat_charts_list">
                        <!-- <a href="javascript: window.print();" id="woof_stat_print_btn" class="button button-primary"><?php _e('Print Graphs', 'meta-data-filter') ?></a> -->
...
//***
        ...
        if ($wpdb->query($sql) === false) {
            ?>
        <p class="description"><?php _e("MDTF cannot create database table for statistic! Make sure that your mysql user has the CREATE privilege! Do it manually using your host panel&amp;phpmyadmin!", 'meta-data-filter') ?></p>
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        try {
            ...
            console.log('<?php _e('Google charts library not loaded! If site is on localhost just disable statistic extension in tab Extensions!', 'meta-data-filter') ?>');
        }
        //+++
    ...
    function mdf_stat_draw_graphs() {
        mdf_stat_process_monitor('<?php _e('drawing graphs ...', 'meta-data-filter') ?>');
        ...
        try {
            ...
            mdf_stat_process_monitor('<?php _e('finished!', 'meta-data-filter') ?>');
            jQuery('#mdf_stat_print_btn').show(200);
        } catch (e) {
            console.log('<?php _e('Looks like troubles with JavaScript!', 'meta-data-filter') ?>');
        }
