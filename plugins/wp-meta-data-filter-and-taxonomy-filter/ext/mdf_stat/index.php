...
final class MDF_SEARCH_STAT {
    ...
    public function mdf_stat_check_connection() {
        ...
        try {
            ...
            die(__("Database not connected!    ERROR! ", 'meta-data-filter'));
            // More info!
            //die(__("Database not connected!    ERROR:  ",'meta-data-filter').$e->getMessage());
        }
        die(__("Database successfully connected!!!", 'meta-data-filter'));
    }
    ...
    public function mdf_print_applications_tabs_content() {
        ...
        $localize_script = array(
            ...
            'mdf_stat_leave_empty' => __('leave it empty to see all terms', 'meta-data-filter'),
            'mdf_stat_sel_date_range' => __('Select date range for statistic!', 'meta-data-filter'),
            'mdf_stat_calc' => __('Statistic calculation ...', 'meta-data-filter'),
            'mdf_stat_get_oper_tbls' => __('getting of operative tables ...', 'meta-data-filter'),
            'mdf_stat_oper_tbls_prep' => __('operative tables are prepared', 'meta-data-filter'),
            'mdf_stat_getting_dftbls' => __('getting the data from the table', 'meta-data-filter'),
            'mdf_stat_done' => __('done!', 'meta-data-filter'),
            'mdf_stat_no_data' => __('No data for the selected time period!', 'meta-data-filter'),
        );
        ...
        if (!extension_loaded('pdo_mysql')) {
            echo '<div class="error"><p class="description">' . sprintf(__('PHP extension PDO_MYSQL is not enabled on your server, not possible to collect statistic data! Contact your hosting support to about enabling PDO_MYSQL.', 'meta-data-filter')) . '</p></div>';
        }
        ...
        if (version_compare($memory, $min_memory_mb, '<')) {
            echo '<div class="error"><p class="description">' . sprintf(__('Very recommend for the statistic not less than %s of the memory to avoid malfunctionality. Now is on the site %s', 'meta-data-filter'), size_format($min_memory_mb), size_format($memory)) . '</p></div>';
        }
        //***
            ...
            if (extension_loaded('pdo_mysql')) {
                ...
                if (!empty($pdo_options['host']) AND ! empty($pdo_options['host_db_name']) AND ! empty($pdo_options['host_user'])) {
                    try {
                        ...
                        echo '<div class="error"><p class="description">' . sprintf(__('Wrong data for "<b>Server options for statistic stock</b>" options. Please fill right data AND click "Check DB connection" button and if all right Save the plugin options.', 'meta-data-filter')) . '</p></div>';
                    }
                }
            }
        }
                ...
                if ($key == $key_id) {
                    switch ($val['type']) {
                        case 'checkbox':
                            if ($value) {
                                ...
                                return __('Yes', 'meta-data-filter');
                            } else {
                                return __('No', 'meta-data-filter');
                            }
                            break;
                        case 'label':
                        ...
                        case "slider":
                            ...
                            if (count($range) > 1 AND ( !empty($range[1])AND $range[1] != 0 )) {
                                return sprintf(__("From:%s to: %s", 'meta-data-filter'), $range[0], $range[1]);
                            } else {
                                return $range[0];
                            }
                            break;
                    ...
                    if (!empty($value)) {
                        if ($val['type'] == 'calendar') {
                            ...
                            $range = explode("^", $value);
                            if (count($range) > 1 AND ( !empty($range[1])AND $range[1] != 0 )) {
                                return sprintf(__("From:%s to: %s", 'meta-data-filter'), date("m.d.y", intval($range[0])), date("m.d.y", intval($range[1])));
                            } else {
                                return __('From:', 'meta-data-filter') . date("m.d.y", intval($range[0]));
                            }
                        } else {
                            return __('From:', 'meta-data-filter') . $value;
                        }
                    }
                } elseif ($key . '_to' == $key_id) {
                    if (!empty($value)) {
                        if ($val['type'] == 'calendar') {
                            return __('To:', 'meta-data-filter') . date("m.d.y", intval($value));
                        } else {
                            return __('To:', 'meta-data-filter') . $value;
                        }
                    }
                }
            }
