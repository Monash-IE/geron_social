...
?>
<div class="mdf_shortcode_container mdf_shortcode_skin_<?php echo $shortcode_options['options']['skin'] ?>">
    <form method="get" action="" data-unique-id="<?php echo $uniqid; ?>" data-slug="<?php echo $slug ?>" data-shortcode-id="<?php echo $shortcode_id ?>" id="meta_data_filter_<?php echo $uniqid ?>" class="mdf_search_form <?php if ($shortcode_options['options']['ajax_auto_submit'] == 1 AND $ajax_results): ?>mdf_ajax_content_redraw<?php endif; ?> <?php if ($shortcode_options['options']['ajax_auto_submit'] == 1 AND ! false): ?>mdf_ajax_auto_recount<?php endif; ?> mdf_shortcode_form mdf_shortcode_form_<?php echo $shortcode_id ?>">
        ...
        $widget_options = array(
            ...
            'shortcode_options' => (isset($shortcode_options['options']) ? $shortcode_options['options'] : 0),
        );
        ...
        if (!empty($html_items) AND is_array($html_items)) {
            ...
            foreach ($html_items as $filter_id => $h_items) {
                ...
                <div class="mdf_input_container_block mdf_input_container_block_<?php echo $css_block_counter ++ ?>" <?php if ($this_block_is_hidden): ?>style="display: none;"<?php endif; ?>>
                    <?php
                    if (isset($h_items['post_title']) AND ! empty($h_items['post_title']) AND $h_items['post_title']{0} !== '~'):
                                    ...
                                    case 'by_author' :
                                        ...
                                        <div class="mdf_input_container mdf_by_author_container_<?php echo $meta_key ?> " >
                                            ...
                                            <select name="mdf[<?php echo $meta_key ?>]" id="<?php echo $meta_key ?>" class="mdf_filter_select" >
                                                <option value="-1"> <?php _e("Select author", 'meta-data-filter') ?></option>
                                                <?php
                                        ...
                                        <div class="mdf_input_container_taxonomies">
                                            <?php foreach ($shortcode_options['options']['taxonomies'] as $tax_name => $v) : ?>
                                                ...
                                                <div class="mdf_input_container mdf_taxonomy_<?php echo $tax_name ?> <?php if ($widget_options['act_without_button']): ?>mdf_tax_auto_submit<?php endif; ?> <?php if (isset($widget_options['ajax_items_recount']) AND $widget_options['ajax_items_recount']): ?>mdf_tax_ajax_autorecount<?php endif; ?>" <?php if ($this_item_is_hidden): ?>style="display: none;"<?php endif; ?>>
                                                    ...
                                                    <?php
                                                    if (isset($item['name']{0}) AND $item['name']{0} !== '~' AND $show_how != 'checkbox'):
                                                        $mdtf_title_sections = "h5";
                                                        $mdtf_title_sections = apply_filters('mdf_tag_title_sections', $mdtf_title_sections);
                                                        ?>
                                                    ...
                                                    if (self::is_page_mdf_data()) {
                                                        ...
                                                        if ($show_how == 'checkbox') {
                                                            ...
                                                            echo $title;
                                                            ?>:</<?php echo $mdtf_title_sections ?>>
                                                            <span style="display: inline-block; <?php if (!empty($section_max_height)): ?>max-height: <?php echo $section_max_height ?>px;<?php endif; ?>" class="mdf_tax_filter_section <?php if ($widget_options['act_without_button']): ?>mdf_tax_auto_submit<?php endif; ?>">
                                                                <?php
                                                                $parent_id = 0;
        ...
        <?php if (!$ajax_results): ?>
            <div class="mdf_shortcode_submit_button">
                <?php
                $filter_button_text = __("Filter", 'meta-data-filter');
                ?>
...
//+++
    ...
    <script type="text/javascript">
        jQuery(function () {
            mdf_init_search_form("<?php echo $uniqid ?>", "<?php echo $slug ?>", "<?php echo $search_url ?>", false, <?php echo $shortcode_options['options']['ajax_auto_submit'] ?>);
        });
    </script>
