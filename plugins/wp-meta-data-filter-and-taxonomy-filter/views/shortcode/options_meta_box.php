...
<table class="form-table">
    <tbody>
        ...
        <tr valign="top">
            <th scope="row"><?php _e('Ignore filter-category in wp_query', 'meta-data-filter') ?><br /></th>
            <td>
                ...
                <i><?php _e('Check this if you want to make searching by title/content in all posts of the selected post type slug!', 'meta-data-filter') ?></i>
            </td>
        </tr>
        <tr valign="top">
        ...
        <tr valign="top">
            ...
            <td>
                ...
                <?php _e('To use this option please uncheck "Auto submit". Use it to stay on the same page and see results filtered by AJAX.', 'meta-data-filter') ?>
            </td>
        </tr>
        <tr valign="top">
                    ...
                    <ul style="margin-left: 15px;" id="mdf_shotcode_taxonomies">
                        <?php foreach ($output_tax_array as $tax_key => $tax_name) : ?>
                            <li style="padding: 5px; border: solid 1px #ccc; margin-bottom: 11px;">
                                ...
                                <input type="text" style="width: 400px;" placeholder="<?php _e('taxonomy custom title', 'meta-data-filter') ?>" name="shortcode_options[options][taxonomies_title][<?php echo $tax_key ?>]" value="<?php echo @$shortcode_options['options']['taxonomies_title'][$tax_key] ?>">&nbsp;
                                <input type="text" style="width: 120px;" placeholder="<?php _e('block max-height', 'meta-data-filter') ?>" name="shortcode_options[options][taxonomies_checkbox_maxheight][<?php echo $tax_key ?>]" value="<?php echo @$shortcode_options['options']['taxonomies_checkbox_maxheight'][$tax_key] ?>">&nbsp;
                                <?php
                                ...
                                //***
                                $types = array_merge(self::$tax_items_types, array('multi_select' => __('Multiple checkbox select', 'meta-data-filter')));
                                ?>
                                ...
                                <input type="text" class="text" style="width: 100%;" placeholder="<?php printf(__('exluded terms ids for %s. Example: 11,22,33', 'meta-data-filter'), $tax_name) ?>" name="shortcode_options[options][excluded_terms][<?php echo $tax_key ?>]" value="<?php echo @$shortcode_options['options']['excluded_terms'][$tax_key] ?>"><br />
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
