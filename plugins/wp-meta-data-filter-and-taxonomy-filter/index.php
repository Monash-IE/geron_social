...
//for compatibility only
    ...
    public static function plugin_action_links($links) {
        return array_merge(array(
            ...
            '<a target="_blank" href="' . esc_url('http://wp-filter.com/documentation/') . '">' . __('Documentation', 'meta-data-filter') . '</a>'
                ), $links);
    }
...
//print_r($widget_options);
    ...
    private static function draw_term_childs_label($terms, $mdf_parent_id = 0, $selected_ids = array(), $hide, $tax_name = 0, $is_ajax = true, $hide_string_ids = '', $widget_options = array()) {
        if (!empty($terms)) {
            $show_tax_all_childs = (int) self::get_setting('show_tax_all_childs');
            ?>
    ...
    private static function recursive_show_checkboxеs($terms, $term_id = 0, $selected_ids = array(), $hide, $tax_name = 0, $is_ajax = true, $hide_string_ids = "", $widget_options = array(), $child_levl = 0) {
        if (!empty($terms)):
            $show_tax_all_childs = (int) self::get_setting('show_tax_all_childs');
            ?>
            <?php foreach ($terms as $term_id => $term) : ?>
