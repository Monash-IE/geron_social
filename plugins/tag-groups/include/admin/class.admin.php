    ...
    class TagGroups_Admin
    {
        ...
        static function admin_init()
        {
            ...
            if ( current_user_can( $tag_group_role_edit_groups ) ) {
                ...
                add_action( 'wp_ajax_tg_ajax_benchmark', array( 'TagGroups_Settings', 'ajax_benchmark' ) );
            }
        ...
        static function register_menus()
        {
            ...
            foreach ( $tag_group_post_types as $post_type ) {
                ...
                $submenu_page = add_submenu_page(
                    ...
                    array( 'TagGroups_Admin', 'group_administration' )
                );
            }
        }
            ...
            if ( $show_filter_tags ) {
                ...
                if ( isset( $term_filter ) ) {
                    ...
                    if ( '-1' == $term_filter ) {
                        ...
                        add_action(
                            'terms_clauses',
                            array( 'TagGroups_Admin', 'terms_clauses' ),
                            10,
                            3
                        );
                    }
        ...
        public static function update_edit_term_group_edit( $term_id )
        {
            ...
            if ( isset( $_POST['tag-groups-taxonomy'] ) ) {
                ...
                if ( isset( $_POST['description'] ) ) {
                    ...
                    if ( current_user_can( 'unfiltered_html' ) || get_option( 'tag_group_html_description', 0 ) ) {
        ...
        static function add_admin_footer_text( $text )
        {
            ...
            if ( !empty($screen) && 'edit-tags' == $screen->base && get_option( 'tag_group_show_filter_tags', 1 ) && in_array( $screen->taxonomy, $enabled_taxonomies ) ) {
        ...
        static function apply_post_filter( $query )
        {
            ...
            if ( count( $taxonomy_intersect ) && isset( $_GET['tg_filter_posts_value'] ) && $_GET['tg_filter_posts_value'] !== '' ) {
                $query->query_vars['meta_query'] = array( array(
                    'key'     => '_cm_post_terms_' . (int) $_GET['tg_filter_posts_value'],
                    'compare' => 'EXISTS',
                ) );
            }
            return $query;
        ...
        static function ajax_manage_groups()
        {
            ...
            if ( $task == 'refresh' || $task == 'test' || current_user_can( $tag_group_role_edit_groups ) && wp_verify_nonce( $_REQUEST['nonce'], 'tg_groups_management' ) ) {
                ...
                echo  json_encode( array(
                    ...
                    'groups'         => TagGroups_Admin::group_table( $start_position, $end_position, $taxonomy ),
            ...
            if ( strpos( $where, 'tag-groups-settings' ) !== false ) {
                ...
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    wp_register_style(
                        ...
                        TAG_GROUPS_PLUGIN_URL . '/assets/css/backend.css',
                        array(),
                        TAG_GROUPS_VERSION
                    );
                } else {
                    wp_register_style(
                        'tag-groups-css-backend-tgb',
                        TAG_GROUPS_PLUGIN_URL . '/assets/css/backend.min.css',
                        array(),
                        TAG_GROUPS_VERSION
                    );
                }
