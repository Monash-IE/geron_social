    ...
    class TagGroups_Loader
    {
        ...
        public function add_hooks()
        {
            ...
            if ( is_admin() ) {
                 ...
                 */
                add_action( 'delete_term', array( $tag_group_groups, 'clear_tag_groups_group_terms' ), 20 );
                // ### Groups ###
                /**
                 *  After a term group order has possibly been changed, we should sort the groups in the term meta
                 */
                add_action( 'term_group_saved', array( $tag_group_terms, 'sort_groups' ) );
                /**
        ...
        public function enqueue_scripts()
        {
            ...
            global  $post, $tag_groups_premium_fs_sdk ;
            
            if ( get_option( 'tag_group_shortcode_enqueue_always', 1 ) || !is_a( $post, 'WP_Post' ) || has_shortcode( $post->post_content, 'tag_groups_cloud' ) || has_shortcode( $post->post_content, 'tag_groups_accordion' ) || has_shortcode( $post->post_content, 'tag_groups_table' ) || has_shortcode( $post->post_content, 'tag_groups_tag_list' ) || has_shortcode( $post->post_content, 'tag_groups_alphabet_tabs' ) || strpos( $post->post_content, '<!-- wp:chatty-mango/tag-groups-cloud-tabs' ) !== false || strpos( $post->post_content, '<!-- wp:chatty-mango/tag-groups-cloud-accordion' ) !== false || strpos( $post->post_content, '<!-- wp:chatty-mango/tag-groups-alphabet-tabs' ) !== false ) {
                    ...
                    if ( file_exists( WP_CONTENT_DIR . '/uploads/' . $theme . '/jquery-ui.theme.min.css' ) ) {
                        ...
                        if ( file_exists( WP_CONTENT_DIR . '/uploads/' . $theme . '/jquery-ui.theme.css' ) ) {
                            ...
                            if ( file_exists( WP_CONTENT_DIR . '/uploads/' . $theme ) ) {
                                ...
                                error_log( '[Tag Groups] Error finding ' . WP_CONTENT_DIR . '/uploads/' . $theme );
                            }
