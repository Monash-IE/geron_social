...
jQuery( window ).load(function() {
    ...
    if(typeof ol !== 'undefined'){
        const ep_maps = [];
        const ep_vectorSources = [];
        const ep_pop_elements = [];
        const ep_popups = [];
        const ep_features = [];
        const ep_icons = [];
        const ep_proj_source = new ol.proj.Projection({code: 'EPSG:4326'});
        const ep_proj_destination = new ol.proj.Projection({code: 'EPSG:900913'});
        const ep_interactions = eventpost_params.map_interactions;
        const ep_retina = (window.retina || window.devicePixelRatio > 1.5);
        ...
        function ep_focus_feature(map_id, feature){
            ...
            var view = ep_maps[map_id].getView();
            ...
            var html_output = '<a href="' + feature.get('link') + '">' +
        ...
        jQuery('.event_geolist').each(function () {
            ...
            if(jQuery(this).hasClass('has-list')){
                ...
                jQuery('.eventpost-item-list a', jQuery(this)).click(function(e){
                    e.preventDefault();
                    var feature_selector = geo_id+'-'+jQuery(this).parents('.event_item').first().data('id')
            ...
            jQuery(this).find('address').each(function () {
                ...
                if (lat !== undefined && lon !== undefined) {
                    ...
                    var coords = new ol.proj.transform([lon, lat], ep_proj_source, ep_proj_destination);
    ...
    function eventpost_cal_links() {
        jQuery('.eventpost_calendar table td h4').each(function(){
            var event_datas = jQuery(this).next('.event_data');
            jQuery(this).css({
        ...
        jQuery('.eventpost_cal_bt').click(function () {
            ...
            jQuery.get(
                    eventpost_params.ajaxurl,
                    {
                        ...
                        tax_name: calcont.data('tax_name'),
                        tax_term: calcont.data('tax_term')
                    },
                    function (data) {
                        calcont.html(data);
                        eventpost_cal_links();
    ...
    jQuery('.eventpost_calendar').each(function () {
        ...
        jQuery.get(
                eventpost_params.ajaxurl,
                {
                    ...
                    tax_name: jQuery(this).data('tax_name'),
                    tax_term: jQuery(this).data('tax_term')
                },
                function (data) {
                    calcont.html(data);
                    eventpost_cal_links();
