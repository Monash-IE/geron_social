...
class EventPost {
	...
	public function load_styles() {
		...
		// Lib scripts
		wp_enqueue_style('openlayers', plugins_url('/css/openlayers.css', __FILE__), false,  filemtime( "{$this->plugin_path}/css/openlayers.css" ));
	...
	public function load_map_scripts() {
		// JS
		wp_enqueue_script('openlayers', plugins_url('/js/OpenLayers.js', __FILE__), false,  filemtime( "{$this->plugin_path}/js/OpenLayers.js" ), true);
	...
	function get_rich_result($event){
		...
		return apply_filters('event-post-rich-result', array(
			...
			'datePublished'=>$event->post_date_gmt,
			'dateModified'=>$event->post_modified_gmt,
			'startDate'=>date('Y-m-d h:i', $event->time_start),
			'endDate'=>date('Y-m-d h:i', $event->time_end),
...
<?php
	...
	public function add_custom_box() {
		...
		if(!function_exists('shortcode_ui_register_for_shortcode') && !function_exists('register_block_type')){
