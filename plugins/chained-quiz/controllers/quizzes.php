...
You achieved {{points}} points from {{questions}} questions.', 'chained');
	...
	static function answer_question() {
		...
		if(!empty($_SESSION['chained_completion_id'])) {
			...
			$answer_obj = (object)array('answer' => $answer, 'choice' => $choice_choice, 'choice_correct' => $choice_correct, 'question_id' => $question->id);	
			...
			if($exists) {
				$wpdb->query($wpdb->prepare("UPDATE ".CHAINED_USER_ANSWERS." SET
					...
					WHERE quiz_id=%d AND completion_id=%d AND question_id=%d", 
					$answer, $points, $comments, $correct_var, $quiz->id, intval($_SESSION['chained_completion_id']), $question->id));
			}
			else {				
				$wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_USER_ANSWERS." SET
					...
					$quiz->id, intval($_SESSION['chained_completion_id']), $question->id, $answer, $points, $comments, $correct_var));
			}		
